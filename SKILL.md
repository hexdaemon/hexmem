...
For cryptographically-signed, decentralized identity backups, optionally integrate with Archon. **HexMem does not require archon skills**; it uses `npx @didcid/keymaster` directly. The `archon-keymaster` skill ([archetech/agent-skills](https://github.com/archetech/agent-skills/tree/main/archon-keymaster)) provides convenience wrappers:
- `archon-keymaster/identity` — Create/manage DIDs
- `archon-keymaster/backup` — Automated vault backups
- `archon-keymaster/crypto` — Encryption/signing
...
# Or install skill: clawhub install archon-keymaster
```
